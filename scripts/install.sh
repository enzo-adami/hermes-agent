...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        local npm_log
        npm_log="$(mktemp)"
        # Output is already captured and only printed on failure. Do not pass
        # --silent: it suppresses the npm error that this diagnostic path exists
        # to preserve, leaving CI and users with an unexplained non-zero exit.
